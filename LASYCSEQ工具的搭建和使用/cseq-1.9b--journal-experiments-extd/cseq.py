...
FRAMEWORK_VERSION = 'CSeq-2.0-2020.05.08'
...
def run():
	...
	for cseqenv.transforms, m in enumerate(cseqenv.modules):
		...
		try:
			...
			if cseqenv.debug:
				core.utils.saveFile('%s/_%s_input___%s.c' % (cseqenv.debugpath,fileno,m.name()),m.input)
				core.utils.saveFile('%s/_%s_output__%s.c' % (cseqenv.debugpath,fileno,m.name()),m.output)
			...
			if cseqenv.debug and 'markedoutput' in dir(m):   # only if the current module is a Translator
				core.utils.saveFile('%s/_%s_marked__%s.c' % (cseqenv.debugpath,fileno,m.name()),m.markedoutput)
				core.utils.saveFile('%s/_%s_linemap__%s.c' % (cseqenv.debugpath,fileno,m.name()),m.getlinenumbertable())
