...
import datetime, getopt, hashlib, sys, os.path, os, re, glob
...
def _sanitiseinput(input):
	...
	for line in input.splitlines():
		...
		#__thread int need_softirq;
		line = re.sub(r'__thread int (.*);', r'int __cs_thread_local_\1[THREADS+1];', line)

		#__thread unsigned int my_smp_processor_id;
		line = re.sub(r'__thread unsigned int (.*);', r'int __cs_thread_local_\1[THREADS+1];', line)


		########## line = line.replace('*__restrict ','* ',1)
		########## line = line.replace(' __const ',' ',1)
		########## line = line.replace('__inline ',' ',1)
		# fix for void; line
...
def _gnu_extension_fix(input):
	text = input

	''' 3. Fix Statement Expression (SVCOMP16 workaround)
