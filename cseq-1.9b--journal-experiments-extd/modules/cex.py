...
backendERROR['cbmc-528'] = 'CONVERSION ERROR'
...
class cex(core.module.BasicModule):
	...
	def loadfromstring(self, string, env):
		...
		if self.showcex or self.witness:
			...
			if self.svcomp:
				...
				readFile = openedFile.read()  ### readFile = openedFile.read().encode()  <-- python3
			...
			if self.witness is not None: core.utils.saveFile(self.witness,cex)
	...
	def _shortanswer(self,input):
		...
		return "%s, %s, %0.2f, %0.2fMB" % (self.env.inputfile,result,time.time()-self.env.starttime, (self.memsize/1024.0))
