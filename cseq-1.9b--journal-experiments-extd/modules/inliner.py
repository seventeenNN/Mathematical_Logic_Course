...
class inliner(core.module.Translator):
	...
	def visit_ID(self, n):
		...
		# or to a pointer...
		#
		if (self.Parser.isglobal(self.blockid,n.name) and not
	...
	def visit_FuncDef(self,n):
		...
		#print "- - - -> function: %s   overall:%s   explicit:%s   threads:%s" % (f,cntoveralloccurrences,cntexplicitcalls,cntthreads) 
	...
	def visit_FuncCall(self, n):
		...
		if self.visitingfargs > 0 and self.__needsExpandedHere(fref): ## and not fref.startswith('__CSEQ') and not fref.startswith('__cs_') and not fref.startswith('__VERIFIER_'):
			...
			elif not self.Parser.funcIsVoid[fref] and self.stack[-2] == 'Compound':
				s = '<a_function_call_was_here>'   # non-void function but return value not stored
	...
	def visit_Return(self, n):
		if len(self.indexStack) > 0:
			...
			if not self.Parser.funcIsVoid[self.currentFunction[-1]]:
		...
		if (self.currentFunction[-1] in self.Parser.threadName or
			self.currentFunction[-1] == 'main'):
			return 'goto __exit_%s; ' % (self.currentFunction[-1])
	...
	def _inlineFunction(self,fdef,fcall,simple):
		...
		if fcall.args is not None:
			for paramno,expr in enumerate(fcall.args.exprs):   # for each parameter in the function call
				##self.log("analysing parameter %s for function %s: %s :%s)..." % (paramno,fname,self.Parser.funcParams[fname][paramno],self._mapbacklineno(self.currentinputlineno)[0]))

				# Simplification of parameter passing:
				# 	case 1: references to global variables as function parameters
				...
				if (type(expr) == pycparser.c_ast.UnaryOp and
					...
					#self.log("removing reference to global variable &%s from the fuction call" % expr.expr.name)
					#self.log("changing in the function body (*%s) -> (%s)" % (self.Parser.funcParams[fname][paramno], expr.expr.name))
					...
					expr.name not in self.Parser.varNames[self.currentFunction[-1]] and
					expr.name in self.Parser.varNames['']):
					self.debug("%s: simplified passing of global variable '%s' as argument %s in call to '%s'" % (self._mapbacklineno(self.currentinputlineno)[0],expr.name,paramno,fname))
					paramnochange.append(paramno)
				elif type(expr) == pycparser.c_ast.Constant: 
					self.debug("%s: simplified passing of constant value '%s' as argument %s in call to '%s'" % (self._mapbacklineno(self.currentinputlineno)[0],expr.value,paramno,fname))
					paramnochange.append(paramno)
		...
		if not noargs:
			linefcall = self._mapbacklineno(self.currentinputlineno)[0]

			for cnt in range(0,apcnt):
				#fpold = self.Parser.funcParams[fname][cnt]
				#fp = self._generate_decl(fdef.decl.type.args.params[cnt])
				...
				if self.simplifyargs and type(fdef.decl.type.args.params[cnt]) != pycparser.c_ast.EllipsisParam:
					if (type(fdef.decl.type.args.params[cnt].type) == pycparser.c_ast.PtrDecl and
						...
						#
						try:
							#self.log("---------- line %s scanning passed parameters for call to function %s" % (linefcall,fname))
							...
							if overalldereferences == overalloccurrences:
								...
								line = self._mapbacklineno(self.currentinputlineno)[0]
								#self.log("line %s: simplifying call to %s(no.%s)" % (linefcall,fname,cnt))
								#self.log("line %s: function defined here" % line)
								#self.log("          ap[%s] %s" % (cnt,ap))
								#self.log("          fp[%s] %s" % (cnt,fp))
								#self.log("         ap1[%s] %s" % (cnt,ap1))
								#self.log("         fp1[%s] %s" % (cnt,fp1))
								#self.log("         fp2[%s] %s" % (cnt,fp2))
								#self.log("========== simplifying line:%s function call %s argument no.%s (%s)" % (line,fname,cnt,self.Parser.funcParams[fname][cnt]))

								# 1. change in the function body 
								####self.switchTo[-1][self.Parser.funcParams[fname][cnt]] = fpid
								#self.switchTo[-1][self.Parser.funcParams[fname][cnt]] = '&'+ap1
		...
		#
		if not self.Parser.funcIsVoid[fname]:
			#fOutput = self._make_indent()+'static %s __cs_retval_%s;\n' % (self.Parser.funcBlockOut[fname], self.indexStack[-1])
		...
		#inlined = self._shiftIndent(self.visit(self.Parser.funcASTNode[fname].body))
		...
		# bottom
		if self.Parser.funcExitPointsCnt[fname] == 0 and self.Parser.funcIsVoid[fname]:
			pass  # function is void and contains no return statement, don't need an exit label to simulate returns
		elif self.Parser.funcExitPointsCnt[fname] == 0 and not self.Parser.funcIsVoid[fname]:
